...
def find_input_pairs(input_dir):
    ...
    for s1_path in s1_files:
        ...
        s2_path = s1_path.replace('s1', 's2')
        if os.path.exists(s2_cloudy_path) and os.path.exists(s2_path):
            pairs.append((s1_path, s2_cloudy_path, s2_path))
        else:
...
def run_batch_inference(config):
    ...
    for s1_path, s2_cloudy_path, s2_path in tqdm(input_pairs, desc="Reconstructing Images"):
        try:
            # Load data
            s2_cloudy_tif, s2_cloudy_img_raw = read_tif(s2_cloudy_path)
            _, s1_img_raw = read_tif(s1_path)
            _, s2_img_raw = read_tif(s2_path)
            ...
            s2_processed = process_MS(s2_img_raw)
            ...
            target_tensor = torch.from_numpy(s2_processed).unsqueeze(0).unsqueeze(0).to(device)
